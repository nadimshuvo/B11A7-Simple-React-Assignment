...
const Favorite_Items_Section = ({ favorites,totalBidsAmount, onFavoriteDeleteIconClick }) => {
  

  return (
    <div className="min-w-[350px] xl:min-w-[400px] 2xl:min-w-[488px] bg-white text-black rounded-3xl mt-8 border border-[#DCE5F3] shadow-2xl">
      ...
      {favorites.length > 0 ? (
        favorites.map((item,index) => (
          <Favorite_Items key={item.id} item={item} onFavoriteDeleteIconClick={onFavoriteDeleteIconClick}
          isLastItem={index === favorites.length - 1}/>
        ))
      ) : (
        <No_favorites />
      )}

      <div className="text-[26px] leading-8 text-black flex justify-center gap-3 2xl:gap-[76px] pb-8 pt-6 border-t border-[#DCE5F3]">
        <h2 className="sora-regular">Total bids Amount</h2>
        <p className="sora-medium">${totalBidsAmount}</p>
      </div>
    </div>
  );
};
